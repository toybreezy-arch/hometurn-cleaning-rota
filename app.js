...
// From September onward, HomeTurn makes a shared random mix each week.
// The shuffle is seeded by the week number, so everyone sees the same rota.
...
function shuffledPeople(weekNumber) {
  let seed = 202609 + weekNumber * 7919;
  const random = () => { seed = (seed * 1664525 + 1013904223) >>> 0; return seed / 4294967296; };
  const mix = [...people];
  for (let i = mix.length - 1; i > 0; i--) { const j = Math.floor(random() * (i + 1)); [mix[i], mix[j]] = [mix[j], mix[i]]; }
  return mix.slice(0, jobs.length);
}
const futureRota = Array.from({ length: 104 }, (_, index) => ({
  date: addWeeks(rota[rota.length - 1].date, index + 1),
  assignments: shuffledPeople(index + 1)
}));
...
document.querySelector("#schedule").innerHTML = fullRota.map((week,index) => `<article class="week ${index===active?"current-week":""}"><h3>From ${shortFormat.format(weekDate(week.date))}${index >= rota.length ? " · Random mix" : ""}</h3>${week.assignments.map((person,i)=>`<p><b>${jobs[i]}</b>${person}</p>`).join("")}</article>`).join("");
