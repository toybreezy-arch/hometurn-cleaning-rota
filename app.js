...
function currentIndex() { const first = weekDate(rota[0].date); const days = Math.floor((today-first)/86400000); return days < 0 ? 0 : Math.floor(days/7) % rota.length; }
const active = currentIndex();
document.querySelector("#current-title").textContent = `Week commencing ${dateFormat.format(weekDate(rota[active].date))}`;
document.querySelector("#currentRota").innerHTML = rota[active].assignments.map((person,i) => `<div class="assignment"><span>${jobs[i]}</span><strong>${person}</strong></div>`).join("");
document.querySelector("#schedule").innerHTML = rota.map((week,index) => `<article class="week ${index===active?"current-week":""}"><h3>From ${shortFormat.format(weekDate(week.date))}</h3>${week.assignments.map((person,i)=>`<p><b>${jobs[i]}</b>${person}</p>`).join("")}</article>`).join("");
...
document.querySelector("#week").innerHTML = rota.map(week=>`<option value="${week.date}">Week commencing ${dateFormat.format(weekDate(week.date))}</option>`).join("");
